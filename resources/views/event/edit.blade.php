    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm p-6">
            <form action="{{ route('event.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5"
                x-data="{ tipe: '{{ old('tipe', $event->tipe ?? 'offline') }}' }">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="pemateri" value="{{ old('pemateri', $event->pemateri) }}" placeholder="Nama pemateri"
                    ...
                    <div>
                        ...
                        <input type="text" name="moderator" value="{{ old('moderator', $event->moderator) }}" placeholder="Nama moderator"
                            class="w-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="tanggal" name="tanggal" value="{{ old('tanggal', $event->tanggal) }}" required readonly
                    ...
                    <div>
                        ...
                        <input type="text" id="waktu" name="waktu" value="{{ old('waktu', \Illuminate\Support\Str::substr($event->waktu, 0, 5)) }}" required readonly
                            class="w-full border rounded-lg px-3 py-2.5 text-sm outline-none transition cursor-pointer
                                bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100
                                focus:ring-2 focus:ring-green-500 focus:border-transparent
                ...
                <div>
                    ...
                    <input type="text" name="tempat" value="{{ old('tempat', $event->tempat) }}" required
                ...
                <div x-show="tipe === 'online'" x-cloak>
                    ...
                    <input type="url" name="link_online" value="{{ old('link_online', $event->link_online) }}" :required="tipe === 'online'"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                        class="w-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition resize-none">{{ old('deskripsi', $event->deskripsi) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran"
                        class="w-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition">
                        @php $currentMetode = old('metode_pembayaran', $event->metode_pembayaran); @endphp
                        <option value="Gratis" {{ $currentMetode === 'Gratis' ? 'selected' : '' }}>Gratis</option>
                        <option value="Berbayar" {{ $currentMetode === 'Berbayar' ? 'selected' : '' }}>Berbayar</option>
                    </select>
                    @error('metode_pembayaran') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                {{-- Harga --}}
                @php $showHarga = old('metode_pembayaran', $event->metode_pembayaran) === 'Berbayar'; @endphp
                <div id="harga-section" class="{{ $showHarga ? '' : 'hidden' }}">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                @include('event._payment-info-fields', ['event' => $event])
